...
def test_no_tool_selected_skips_check():
    state = permission_check(make_state(tool=None), build_default_registry(), RateLimiter())
    assert state.decision is None

def test_low_risk_tool_gets_allowed():
    state = permission_check(make_state(tool="file_reader"), build_default_registry(), RateLimiter())
    assert state.decision == Decision.ALLOWED

def test_high_risk_tool_needs_approval():
    state = permission_check(
        make_state(tool="send_email", role=Role.ADMIN), build_default_registry(), RateLimiter()
    )
